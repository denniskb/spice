...
std::pair<double, double> bench( ::model m, int nneuron );
    ...
    std::size_t nsyn = fire::arg( "--nsyn" ) )
{
	...
	{
	case ::model::vogels:
		nneuron = static_cast<int>( std::sqrt( nsyn / 0.02 ) );
		break;
	...
	auto t = bench( str2model( model ), nneuron );
	
	std::cout << "\t\"setuptime\": " << t.first << "," << std::endl;
	std::cout << "\t\"simtime\": " << t.second << std::endl;
...
std::pair<double, double> _bench( layout l, int delay )
{
	try
	{
		int const ITER = 10'000;
		double tsetup = -1, tsim = -1;

		timer t;

		cuda::snn<Model> net( l, 0.0001f, delay );
		device::active().synchronize();
		tsetup = t.stop();

		t = timer();
		for( int i = 0; i < ITER / ( std::is_same_v<Model, brunel_with_plasticity> ? 1 : delay ); i++ )
			net.step();

		device::active().synchronize();
		tsim = t.stop();

		return { tsetup, tsim * 1.0 };
	}
	catch( ... )
	{
		return { -1, -1 };
...
bench( ::model m, int nneuron )
{
	try
	{
		switch( m )
		{
		case ::model::vogels: return _bench<vogels_abbott>( { nneuron, 0.02f }, 8 );
		case ::model::brunel:
			return _bench<brunel>(
			    { { nneuron / 2, nneuron / 2 }, { { 0, 1, 0.1f }, { 1, 1, 0.1f } } },
			    15 );
		case ::model::brunel_plus:
