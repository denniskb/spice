...
#define span_CONFIG_CONTRACT_LEVEL_EXPECTS_ONLY 1   // only check precond.
#define span_CONFIG_NO_EXCEPTIONS 1                 // disable exceptions
#define span_CONFIG_CONTRACT_VIOLATION_THROWS 0     // disable exceptions
#define span_CONFIG_CONTRACT_VIOLATION_TERMINATES 1 // use std::terminate upon contract violation
#define span_FEATURE_MEMBER_AT 1
...
#ifndef span_CONFIG_count_tYPE
#define span_CONFIG_count_tYPE std::ptrdiff_t
#endif
...
typedef span_CONFIG_count_tYPE count_t;
...
inline void throw_out_of_range( std::size_t idx, std::size_t size )
{
...
template <class T>
inline span_constexpr std::size_t to_size( T size )
{
	return static_cast<count_t>( size );
}
...
public:
	...
	span_constexpr_exp span( pointer ptr, count_type count )
	    : data_( ptr )
	    , size_( count )
	{
		span_EXPECTS(
...
#if span_HAVE( DEFAULT_FUNCTION_TEMPLATE_ARG )
	template <count_type Offset, extent_type Count = dynamic_extent>
#else
	template <count_type Offset, extent_type Count /*= dynamic_extent*/>
#endif
	...
	subspan( count_type offset, count_type count = static_cast<count_type>( dynamic_extent ) ) const
	{
		...
		return span<element_type, dynamic_extent>(
		    data() + offset,
		    count == static_cast<count_type>( dynamic_extent ) ? size() - offset : count );
	}

	// 26.7.3.4 Observers [span.obs]

	span_constexpr count_type size() const span_noexcept { return size_; }

	span_constexpr count_type size_bytes() const span_noexcept
	{
	...
	span_constexpr_exp reference operator[]( count_type idx ) const
	{
	...
	span_constexpr14 reference at( count_type idx ) const
	{
#if span_CONFIG( NO_EXCEPTIONS )
...
template <class T>
span_constexpr auto first( T & t, std::size_t count ) -> decltype( make_span( t ).first( count ) )
{
...
template <std::size_t Offset, extent_t Count = dynamic_extent, class T>
...
template <class T>
span_constexpr auto subspan( T & t, std::size_t offset, extent_t count = dynamic_extent )
...
{
template <class T>
inline span_constexpr span<T> make_span( T * ptr, std::size_t count ) span_noexcept
{
...
{
template <class T>
inline span_constexpr auto byte_span( T & t ) span_noexcept -> span<detail::byte, span_sizeof( T )>
{
